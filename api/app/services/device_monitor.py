...
def check_device_timeout():
    ...
    try:
        interval = int(ConfigCache.get("env_log_interval_sec"))
        now = datetime.now(timezone.utc)
        runtimes = db.query(RoomRuntimeStatus).filter(
            RoomRuntimeStatus.device_running == True
        ).all()

        for r in runtimes:
            last_activity = r.last_env_log or r.updated_at

            if not last_activity:
                r.device_running = False
                continue
            # UTC Aware Datetime
            ...
            if delta > interval * 2:
                print(f"!!! DEVICE TIMEOUT: Room {r.room_id} (Delta: {delta:.0f}s)")
                r.device_running = False
                
        db.commit()
    except Exception as e:
        print(f"Error in check_device_timeout: {e}")
    finally:
        db.close()